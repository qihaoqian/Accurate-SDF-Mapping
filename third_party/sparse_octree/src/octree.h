...
enum OcType { NONLEAF = -1, SURFACE = 0, FEATURE = 1 };
...
public:
    ...
    void
    set_feature_index_() {
        feature_index_ = feature_next_index_++;
    }

    void
    set_node_index_() {
        node_index_ = next_node_index_++;
    }

    Octant *&
    child(const int x, const int y, const int z) {
        return child_ptr_[x + y * 2 + z * 4];
    };

    Octant *&
    child(const int offset) {
        return child_ptr_[offset];
    }
    ...
    Octant *child_ptr_[8];
    int node_index_;
    static int next_node_index_;
    int feature_index_;
    static int feature_next_index_;
};
...
public:
    ...
    ~Octree();
    void
    init(int64_t grid_dim, int64_t grid_num, double voxel_size, int64_t full_depth);

    // allocate voxels
    std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor>
    insert(torch::Tensor vox);

    // find a particular octant
    Octant *
    find_octant(std::vector<float> coord);

    // test intersections
    bool
    has_voxel(torch::Tensor pose);

    // query features
    torch::Tensor
    get_features(torch::Tensor pts);

    // get all voxels
    torch::Tensor
    get_voxels();
    std::vector<float>
    get_voxel_recursive(Octant *n);

    // get leaf voxels
    torch::Tensor
    get_leaf_voxels();
    std::vector<float>
    get_leaf_voxel_recursive(Octant *n);

    // count nodes
    int64_t
    count_nodes();
    int64_t
    count_recursive(Octant *n);
    ...
    int64_t
    leaves_count_recursive(Octant *n);
...
private:
    ...
    count_nodes_internal();
    std::pair<int64_t, int64_t>
    count_recursive_internal(Octant *n);
    void
    buildFullTreeRec(Octant *node, int64_t depth, int64_t full_depth);
};
