...
from .voxel_helpers import ray_sample_isdf
...
def masked_scatter(mask, x):
    ...
    return x.new_zeros(B, K, x.size(-1)).masked_scatter(mask.unsqueeze(-1).expand(B, K, x.size(-1)), x)
...
def masked_scatter_ones(mask, x):
    ...
    return x.new_ones(B, K, x.size(-1)).masked_scatter(mask.unsqueeze(-1).expand(B, K, x.size(-1)), x)
...
@torch.enable_grad()
def combined_interpolation(
    sampled_xyz, point_xyz, point_feats, point_vector_features, voxel_size: float, point_voxel_size
):
    """
    ...
    extra_interpolated_features = extra_interp(
        sampled_xyz, point_xyz, point_feats, point_vector_features, voxel_size, point_voxel_size
    )

    # tri-linear interpolation
    p = ((sampled_xyz - point_xyz) / (point_voxel_size * voxel_size) + 0.5).unsqueeze(
        1
    )  # add 0.5 value are clamped to [0,1]
...
def offset_points(point_xyz, quarter_voxel=1, offset_only=False, bits=2):
    ...
    offset = (
        torch.cat([ox.reshape(-1, 1), oy.reshape(-1, 1), oz.reshape(-1, 1)], 1).type_as(point_xyz) - bits
    ) / float(bits - 1)
    if not offset_only:
...
def get_features(sampled_xyz, sampled_idx, map_states, voxel_size):
    ...
    point_sdf_priors = F.embedding(point_emd_idx, sdf_priors_all).view(
        point_xyz.size(0), -1
    )  # (chunk_size,8,emd_dim) -> (chunk_size,8*emd_dim)
    point_vector_features = F.embedding(point_emd_idx, vector_features).view(
        point_xyz.size(0), 8, 3
    )  # (chunk_size,8,3)
    # sdf_priors = get_embeddings(sampled_xyz, point_xyz, point_sdf_priors, point_vector_features, point_voxel_size, voxel_size)
    sdf_priors = combined_interpolation(
        sampled_xyz, point_xyz, point_sdf_priors, point_vector_features, voxel_size, point_voxel_size
    )
    feats = None
...
def bundle_adjust_frames(
    ...
    for i, frame in enumerate(keyframe_graph):
        ...
        sample_idx = valid_idx[
            torch.randint(low=0, high=int(valid_idx.shape[0]), size=(int(num_iterations * (N_rays / num_keyframe)),))
        ][:, 0]
    ...
    (
        ...
        N=sample_specs["N"],
        M=sample_specs["M"],
        d_min=sample_specs["d_min"],
        delta=sample_specs["delta"],
        sigma_s=sample_specs["sigma_s"],
        max_depth=sample_specs["max_depth"],
    )
    sampled_xyz = sampled_xyz.reshape(-1, 3)
    ...
    for _ in range(num_iterations):
        ...
        for i in range(0, num_points, chunk_size):
            chunk_samples = {name: s[i : i + chunk_size] for name, s in samples.items()}
            chunk_inputs = get_features(
                chunk_samples["sampled_point_xyz"], chunk_samples["sampled_point_voxel_idx"], map_states, voxel_size
            )
        ...
        # Combine all outputs
        field_outputs = {name: torch.cat([r[name] for r in field_outputs], dim=0) for name in field_outputs[0]}
