...
@torch.no_grad()
def ray_sample_isdf(
    rays_d_all, rays_o_all, depth_samples_all,
    N=20, M=8, d_min=0.07, delta=0.10, sigma_s=0.05, max_depth=10.0,
    device=None
):
    """
    ...
    Args:
        ...
        N: Number of stratified samples (default: 20)
        M: Number of Gaussian samples around surface (default: 8)
        d_min: Minimum depth value (default: 0.07m)
        delta: Additional range beyond surface (default: 0.10m)
        sigma_s: Standard deviation for Gaussian sampling (default: 0.10m)
    ...
    valid_mask = (depth_samples_all > 0)& (depth_samples_all < max_depth) & torch.isfinite(depth_samples_all)  # (num_rays,)
    ...
    d_max = depth_samples_valid - delta  # (num_valid_rays,)
    # Ensure d_max > d_min
    d_max = torch.where(d_max <= d_min, d_min + delta, d_max)
    
    d_range = d_max - d_min  # (num_valid_rays,)

    # Generate stratified samples for valid rays only
    bin_size = d_range.unsqueeze(1) / N  # (num_valid_rays, 1)
    bin_indices = torch.arange(N, device=device, dtype=torch.float32).unsqueeze(0)  # (1, N)
    bin_starts = d_min + bin_indices * bin_size  # (num_valid_rays, N)
    
    # Uniform random samples within each bin
    uniform_samples = torch.rand(num_valid_rays, N, device=device)  # (num_valid_rays, N)
    stratified_depths = bin_starts + uniform_samples * bin_size  # (num_valid_rays, N)
    
    # Record which stratified samples are beyond surface
    stratified_positive_mask = stratified_depths > depth_samples_valid.unsqueeze(1)  # (num_valid_rays, N)
    
    # 2. Gaussian sampling (vectorized) - perturbation by depth
    gaussian_depths = torch.normal(
        mean=depth_samples_valid.unsqueeze(1).expand(-1, M),  # (num_valid_rays, M)
        std=sigma_s
    )
    ...
    gaussian_positive_mask = gaussian_depths > depth_samples_valid.unsqueeze(1)  # (num_valid_rays, M)
    
    # 3. Surface samples
    surface_samples = depth_samples_valid.unsqueeze(1)  # (num_valid_rays, 1)
    
    # Combine all samples
    all_depths = torch.cat([stratified_depths, gaussian_depths, surface_samples], dim=1)  # (num_valid_rays, N+M+1)
    ...
    # Perturbation mask: Gaussian samples (M samples before surface)
    perturbation_mask = torch.zeros(num_valid_rays, total_samples, dtype=torch.bool, device=device)
    perturbation_mask[:, N:N+M] = True  # M samples before surface
    
    # Free space mask: stratified samples (first N samples)
    ray_sample_mask = torch.zeros(num_valid_rays, total_samples, dtype=torch.bool, device=device)
    ray_sample_mask[:, :N] = True  # First N samples are stratified/free space
    
    # Original negative_sdf_mask for backward compatibility
    negative_sdf_mask = torch.cat([stratified_positive_mask, gaussian_positive_mask, torch.zeros(num_valid_rays, 1, dtype=torch.bool, device=device)], dim=1)  # (num_valid_rays, N+M+1)
    positive_sdf_mask = torch.cat([~stratified_positive_mask, ~gaussian_positive_mask, torch.zeros(num_valid_rays, 1, dtype=torch.bool, device=device)], dim=1)  # (num_valid_rays, N+M+1)
    
    # Calculate 3D coordinates (vectorized)
    sampled_xyz = rays_o_valid.unsqueeze(1) + all_depths.unsqueeze(2) * rays_d_valid.unsqueeze(1)
    
    return sampled_xyz, positive_sdf_mask, negative_sdf_mask, gaussian_positive_mask, surface_mask, perturbation_mask, ray_sample_mask, valid_indices
...
def edge_favored_samples(bound, U, alpha=0.5, seed=None):
    ...
    lows = bound[:, 0]
    highs = bound[:, 1]
    samples = lows + (highs - lows) * u  # broadcast to (U,3)
    return samples