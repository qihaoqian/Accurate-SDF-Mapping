...
def load_and_extract_mesh(ckpt_path, args, mesh_res):
    ...
    mesh, mesh_priors, sdf_u, prior_u, decoder_u = mapper.extract_mesh(res=mesh_res, map_states=mapper.map_states)
...
def rgba_visualize(points, values, alpha=1.0, save_path=None):
    ...
    # 分段赋值（与上面标量逻辑一致）
    r = np.where(i <= 1, n, np.where(i == 2, 0.0, np.where(i == 3, 0.0, np.where(i == 4, n, 1.0))))
    g = np.where(i <= 1, 0.0, np.where(i == 2, n, np.where(i == 3, 1.0, np.where(i == 4, 1.0, n))))
    b = np.where(i <= 1, 1.0, np.where(i == 2, 1.0, np.where(i == 3, n, 0.0)))
...
def get_distance_metrics(
    gt_mesh: trimesh.Trimesh, reconstructed_mesh: trimesh.Trimesh, n_samples=200_000, threshold=0.2, args=None
):
    """
...
def evaluate_mesh(args):
    ...
    recon_prior_mesh_path = os.path.join(
        args.log_dir, args.exp_name, "mesh", f"mesh_{args.mapper_specs['mesh_res']}_priors.obj"
    )
    ...
    if not os.path.exists(recon_mesh_path):
        reconstructed_mesh, reconstructed_mesh_priors = load_and_extract_mesh(
            ckpt_path, args, mesh_res=args.mapper_specs["mesh_res"]
        )
    else:
    ...
    print("--------------------------------")
    print(
        f"Chamfer distance: {mesh_metrics_dict['chamfer_distance']*100:.2f} cm, F1 score: {mesh_metrics_dict['f1_score']*100:.2f}%"
    )
    print(f"Precision: {mesh_metrics_dict['precision']*100:.2f}%, Recall: {mesh_metrics_dict['recall']*100:.2f}%")
    print(
        f"Complete ratio: {mesh_metrics_dict['complete_ratio']*100:.2f} ± {mesh_metrics_dict['complete_ratio_std']*100:.2f}"
    )
    ...
    print("--------------------------------")
    print(
        f"Chamfer distance priors: {mesh_metrics_dict_priors['chamfer_distance']*100:.2f} cm, F1 score priors: {mesh_metrics_dict_priors['f1_score']*100:.2f}%"
    )
    print(
        f"Precision priors: {mesh_metrics_dict_priors['precision']*100:.2f}%, Recall priors: {mesh_metrics_dict_priors['recall']*100:.2f}%"
    )
    print(
        f"Complete ratio priors: {mesh_metrics_dict_priors['complete_ratio']*100:.2f} ± {mesh_metrics_dict_priors['complete_ratio_std']*100:.2f}"
    )
    print(
        f"Complete priors: {mesh_metrics_dict_priors['complete']*100:.2f} ± {mesh_metrics_dict_priors['complete_std']*100:.2f}"
    )
    print(
        f"Accuracy priors: {mesh_metrics_dict_priors['accuracy']*100:.2f} ± {mesh_metrics_dict_priors['accuracy_std']*100:.2f}"
    )
    print(
        f"GT points priors: {mesh_metrics_dict_priors['num_gt_points']}, Pred points priors: {mesh_metrics_dict_priors['num_pred_points']}"
    )
    ...
    if args.eval_depth:
        ...
        print(
            f"Depth L1 priors: {depth_errors_priors.mean()*100:.2f} cm, Depth L1 std priors: {depth_errors_priors.std()*100:.2f}"
        )
    ...
    with open(metrics_file_path, "w", encoding="utf-8") as f:
        ...
        f.write(
            f"Complete Ratio: {recon_mesh_metrics['complete_ratio']*100:.2f} ± {recon_mesh_metrics['complete_ratio_std']*100:.2f}%\n"
        )
        f.write(
            f"Complete: {recon_mesh_metrics['complete']*100:.2f} ± {recon_mesh_metrics['complete_std']*100:.2f} cm\n"
        )
        f.write(
            f"Accuracy: {recon_mesh_metrics['accuracy']*100:.2f} ± {recon_mesh_metrics['accuracy_std']*100:.2f} cm\n"
        )
        ...
        f.write(
            f"Complete Ratio Priors: {recon_mesh_priors_metrics['complete_ratio']*100:.2f} ± {recon_mesh_priors_metrics['complete_ratio_std']*100:.2f}%\n"
        )
        f.write(
            f"Complete Priors: {recon_mesh_priors_metrics['complete']*100:.2f} ± {recon_mesh_priors_metrics['complete_std']*100:.2f} cm\n"
        )
        f.write(
            f"Accuracy Priors: {recon_mesh_priors_metrics['accuracy']*100:.2f} ± {recon_mesh_priors_metrics['accuracy_std']*100:.2f} cm\n"
        )
