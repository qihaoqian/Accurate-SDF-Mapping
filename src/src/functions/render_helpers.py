...
@torch.no_grad()
def get_scores(sdf_network, map_states, voxel_size, bits=8, model="parallel_hash_net", return_sdf_priors=True):
    """
    ...
    def get_scores_once(feats, points):
        ...
        if return_sdf_priors:
            return field_inputs['sdf_priors'][:, -1].reshape(-1, res ** 3, 1).detach().cpu()
        else:
...
def bundle_adjust_frames(
    ...
    childrens = map_states["voxel_structure"]
    import numpy as np
    np.savetxt("centres.txt", centres.cpu().numpy(), fmt='%.2f')
    np.savetxt("childrens.txt", childrens.cpu().numpy(), fmt='%d')
    intersections, hits = ray_intersect(
    ...
    for epoch in range(num_iterations):
        ...
        with torch.cuda.amp.autocast():
            ...
            weights, z_min = sdf2weights(sdf.detach(), truncation, z_vals, sample_mask_per)

            rgb = torch.sum(weights[..., None] * color, dim=-2).detach()
            depth = torch.sum(weights * z_vals, dim=-1).detach()
            final_outputs = {
                "weights": weights,
                "color": rgb,
                "depth": depth,
