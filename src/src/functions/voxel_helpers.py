...
def ray_sample(intersection_outputs, step_size=0.01, fixed=False):
    dists = (
            intersection_outputs["max_depth"] -   #(N_rays, N_hits)
            intersection_outputs["min_depth"]     #(N_rays, N_hits)
    ).masked_fill(intersection_outputs["intersected_voxel_idx"].eq(-1),  #(N_rays, N_hits)
                  0)  # dist 算的是ray在voxel内的距离，下面这个采样等于说就是vox内ray短的少采样
    # 这里dist是ray在voxel内的距离，下面这个采样等于说就是vox内ray短的少采样
