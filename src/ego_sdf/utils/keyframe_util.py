...
def multiple_max_set_coverage(
    ...
    kf_unoptimized_voxels: torch.Tensor | None,
    kf_all_voxels: torch.Tensor | None,
    num_selections: int,
    num_voxels: int,
    device: str,
):
    ...
    Args:
        key_frames (list): Save information about keyframes into a list.
        kf_seen_voxel_num (list): This is a list, each element is the number of the corresponding
                                    voxels contained in the keyframe in key_frames.
        ...
        kf_optimized_voxels (tensor, N + 1): mask of all optimized voxels.
    ...
    if kf_unoptimized_voxels is None:
        ...
        # kf_optimized_voxels.index_fill_(0, voxel_indices, True)
    ...
    while cnt < num_selections:
        result_num = kf_unoptimized_voxels[padded_tensor].long().sum(-1)  # (B)
        value, index = torch.max(result_num, dim=0)
        tqdm.write(f"Select keyframe {index}, cover {value} new voxels")
        selected_frame_indices.append(index.item())
