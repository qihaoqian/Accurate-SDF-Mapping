...
from ego_sdf.ga_trilinear import ga_trilinear
from ego_sdf.octree_config import OctreeConfig
...
class SemiSparseOctree(torch.nn.Module):
    def __init__(self, cfg: OctreeConfig):
        ...
        self.voxels: torch.Tensor = None  # (N, 4) [x, y, z, voxel_size]
        self.voxel_centers: torch.Tensor = None  # (N, 3) in meter
        self.vertex_indices: torch.Tensor = None  # (N, 8) index of vertices, -1 if not exists
        self.structure: torch.Tensor = None  # (N, 9) [children(8), voxel_size]
