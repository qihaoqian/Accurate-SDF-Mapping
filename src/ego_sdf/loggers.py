...
class BasicLogger:
    ...
    def log_raw_image(self, ind, rgb, depth):
        ...
        cv2.imwrite(osp.join(self.img_dir, "{:05d}.jpg".format(ind)), (rgb).astype(np.uint8))
        cv2.imwrite(osp.join(self.img_dir, "{:05d}.png".format(ind)), (depth * 5000).astype(np.uint16))
    ...
    def log_images(self, ind, gt_rgb, gt_depth, rgb, depth):
        ...
        if self.for_eva:
            ...
            if gt_color_np[(gt_depth_np != 0.0) * (depth_np != 0.0)].shape[0] == 0:
                ...
                cv2.imwrite(osp.join(self.img_dir, "frame{:06d}.jpg".format(ind)), color_np * 255)
                cv2.imwrite(
                    osp.join(self.img_dir, "depth{:06d}.png".format(ind)),
                    (depth_np * 1000).astype(np.uint16),
                )  # change to mm
                return None, None, None

            ssim = structural_similarity(
                gt_color_np[(gt_depth_np != 0.0) * (depth_np != 0.0)],
            ...
            cv2.imwrite(osp.join(self.img_dir, "frame{:06d}.jpg".format(ind)), color_np * 255)
            cv2.imwrite(
                osp.join(self.img_dir, "depth{:06d}.png".format(ind)), (depth_np * 1000).astype(np.uint16)
            )  # change to mm
