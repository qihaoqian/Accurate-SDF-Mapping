...
class Mapping:
    def __init__(self, args, logger: BasicLogger, data_stream=None, **kwargs):
        ...
        self.vector_features = torch.zeros(
            (self.num_vertexes, 3),
            requires_grad=self.gradient_augmentation,
            dtype=torch.float32,
        ...
        lr = mapper_specs["lr"]
        self.optimize_params = [{"params": self.decoder.parameters(), "lr": lr}, {"params": self.sdf_priors, "lr": lr}]
    ...
    def run(self, first_frame):
        ...
        mesh, mesh_priors, u, u_priors, u_hash_features = self.extract_mesh(
            res=self.mesh_res, map_states=self.map_states
        )
