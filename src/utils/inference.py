...
def get_sdf_loss(args, sdf_u, prior_u, gt_mesh):
    ...
    faces = gt_mesh.faces.astype(np.uint32)
...
def chamfer_and_hausdorff(pts_a: np.ndarray, pts_b: np.ndarray):
    """Calculate Chamfer / Hausdorff distances based on sampled point sets (Euclidean distance, same units as mesh)"""
    ...
    chamfer = float(d_a2b.mean() + d_b2a.mean())   # For symmetric average, divide by 2
...
def get_distance_metrics(gt_mesh: trimesh.Trimesh, reconstructed_mesh: trimesh.Trimesh,
    ...
    # 4) Calculate metrics
    chamfer_distance, hausdorff_distance = chamfer_and_hausdorff(pts_gt, pts_rec)
...
if __name__ == "__main__":
    ...
    # Calculate SDF loss
    loss_dict = get_sdf_loss(args, sdf_u, prior_u, gt_mesh)
    print("SDF loss:")
    for key, value in loss_dict.items():
        print(f"  {key}: {value:.6f}")

    # Calculate chamfer distance, hausdorff distance
    chamfer_distance, hausdorff_distance = get_distance_metrics(gt_mesh, reconstructed_mesh)
